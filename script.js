...
// true = acepta fotos y videos; false = solo fotos
const ALLOW_VIDEOS   = false;
const MAX_FILE_MB    = 5;
...
// Actualiza checkboxes sin re-renderizar el grid
...
function buildCard(resource, cloudName) {
  ...
  // Toggle centralizado para esta tarjeta
  ...
  checkbox.addEventListener("click", (e) => { e.stopPropagation(); toggleCardSelection(); });
  ...
  // En modo selección toda la imagen actúa como botón; fuera, abre el archivo
  ...
  if (resources.length === 1) {
    ...
    for (let i = 0; i < resources.length; i++) {
      ...
      try {
        ...
        console.warn(`[Upload Gallery] No se pudo incluir: ${filename}`);
      }
    }
...
function showErrorPanel(httpStatus) {
  const errorMap = {
    401: "Tag no encontrado o sin recursos públicos (401).",
    403: "Petición bloqueada (403) — revisar 'Allowed strict referral domains' en Cloudinary.",
...
if (heroSubtitle) {
  const media = ALLOW_VIDEOS ? "imágenes y videos" : "imágenes";
  heroSubtitle.textContent = `Sube, visualiza y descarga ${media}. (Max ${MAX_FILE_MB}MB por archivo)`;
}

const mediaInput = document.getElementById("media-files");
if (mediaInput) {
  mediaInput.accept = ALLOW_VIDEOS ? "image/*,video/*" : "image/*";
}
